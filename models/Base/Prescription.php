...
abstract class Prescription implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Map\\PrescriptionTableMap';
    ...
    /**
     * @var        ObjectCollection|ChildMedicine[] Collection to store aggregation of ChildMedicine objects.
     */
    protected $collMedicines;
    protected $collMedicinesPartial;
     ...
     * @var ObjectCollection|ChildMedicine[]
     */
     ...
     *
     * @return $this|Prescription The current object, for fluid interface
     */
    ...
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[PrescriptionTableMap::COL_ID] = true;
        }
    ...
    public function setPatientId($v)
    {
        ...
        if ($this->patient_id !== $v) {
            $this->patient_id = $v;
            $this->modifiedColumns[PrescriptionTableMap::COL_PATIENT_ID] = true;
        }
    ...
    public function setEmployeeId($v)
    {
        ...
        if ($this->employee_id !== $v) {
            $this->employee_id = $v;
            $this->modifiedColumns[PrescriptionTableMap::COL_EMPLOYEE_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : PrescriptionTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : PrescriptionTableMap::translateFieldName('PatientId', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->medicinesScheduledForDeletion !== null) {
                if (!$this->medicinesScheduledForDeletion->isEmpty()) {
                    foreach ($this->medicinesScheduledForDeletion as $medicine) {
                        // need to save related object because we set the relation to null
                        $medicine->save($con);
                    }
                    $this->medicinesScheduledForDeletion = null;
                }
            }

            if ($this->collMedicines !== null) {
                foreach ($this->collMedicines as $referrerFK) {
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[PrescriptionTableMap::COL_ID] = true;
        if (null !== $this->id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . PrescriptionTableMap::COL_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(PrescriptionTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'ID';
        }
        if ($this->isColumnModified(PrescriptionTableMap::COL_PATIENT_ID)) {
            $modifiedColumns[':p' . $index++]  = 'patient_id';
        }
        if ($this->isColumnModified(PrescriptionTableMap::COL_PRESCRIPTION_DATE)) {
            $modifiedColumns[':p' . $index++]  = 'prescription_date';
        }
    ...
    public function getByPosition($pos)
    {
        switch ($pos) {
            ...
            case 1:
                ...
                break;
            case 2:
                return $this->getPrescriptionDate();
                break;
            case 3:
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            ...
            if (null !== $this->collMedicines) {
                ...
                $result[$key] = $this->collMedicines->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
    ...
    public function setByPosition($pos, $value)
    {
        switch ($pos) {
            ...
            case 1:
                ...
                break;
            case 2:
                $this->setPrescriptionDate($value);
                break;
            case 3:
    ...
    public function fromArray($arr, $keyType = TableMap::TYPE_PHPNAME)
    {
        ...
        if (array_key_exists($keys[2], $arr)) {
            $this->setPrescriptionDate($arr[$keys[2]]);
        }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(PrescriptionTableMap::COL_ID)) {
            $criteria->add(PrescriptionTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(PrescriptionTableMap::COL_PATIENT_ID)) {
            $criteria->add(PrescriptionTableMap::COL_PATIENT_ID, $this->patient_id);
        }
        if ($this->isColumnModified(PrescriptionTableMap::COL_PRESCRIPTION_DATE)) {
            $criteria->add(PrescriptionTableMap::COL_PRESCRIPTION_DATE, $this->prescription_date);
        }
        if ($this->isColumnModified(PrescriptionTableMap::COL_EMPLOYEE_ID)) {
    ...
    public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
    {
        ...
        if ($deepCopy) {
            ...
            foreach ($this->getMedicines() as $relObj) {
                if ($relObj !== $this) {  // ensure that we don't try to copy a reference to ourselves
                    $copyObj->addMedicine($relObj->copy($deepCopy));
                }
            }
    ...
    public function getEmployee(ConnectionInterface $con = null)
    {
        if ($this->aEmployee === null && ($this->employee_id != 0)) {
            ...
            /* The following can be used additionally to
                ...
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aEmployee->addPrescriptions($this);
             */
        }
    ...
    /**
     * Clears out the collMedicines collection
     *
    ...
    {
        $this->collMedicines = null; // important to set this to NULL since that means it is uninitialized
    }

    /**
     * Reset is the collMedicines collection loaded partially.
     */
    public function resetPartialMedicines($v = true)
    {
        $this->collMedicinesPartial = $v;
    }

    /**
     * Initializes the collMedicines collection.
     *
     * By default this just sets the collMedicines collection to an empty array (like clearcollMedicines());
     ...
     * @return void
     */
    public function initMedicines($overrideExisting = true)
    {
        if (null !== $this->collMedicines && !$overrideExisting) {
            return;
        }

        $collectionClassName = MedicineTableMap::getTableMap()->getCollectionClassName();

        $this->collMedicines = new $collectionClassName;
        $this->collMedicines->setModel('\Medicine');
    }

    /**
     * Gets an array of ChildMedicine objects which contain a foreign key that references this object.
     *
     ...
     * @param      ConnectionInterface $con optional connection object
     * @return ObjectCollection|ChildMedicine[] List of ChildMedicine objects
     * @throws PropelException
     */
    public function getMedicines(Criteria $criteria = null, ConnectionInterface $con = null)
    {
        $partial = $this->collMedicinesPartial && !$this->isNew();
        if (null === $this->collMedicines || null !== $criteria  || $partial) {
            if ($this->isNew() && null === $this->collMedicines) {
                // return empty collection
                $this->initMedicines();
            } else {
                $collMedicines = ChildMedicineQuery::create(null, $criteria)
                    ->filterByPrescription($this)
                    ->find($con);

                if (null !== $criteria) {
                    if (false !== $this->collMedicinesPartial && count($collMedicines)) {
                        $this->initMedicines(false);

                        foreach ($collMedicines as $obj) {
                            if (false == $this->collMedicines->contains($obj)) {
                                $this->collMedicines->append($obj);
                            }
                        }

                        $this->collMedicinesPartial = true;
                    }

                    return $collMedicines;
                }

                if ($partial && $this->collMedicines) {
                    foreach ($this->collMedicines as $obj) {
                        if ($obj->isNew()) {
                            $collMedicines[] = $obj;
                        }
                    }
                ...
                $this->collMedicines = $collMedicines;
                $this->collMedicinesPartial = false;
            }
        }

        return $this->collMedicines;
    }

    /**
     * Sets a collection of ChildMedicine objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param      Collection $medicines A Propel collection.
     * @param      ConnectionInterface $con Optional connection object
     * @return $this|ChildPrescription The current object (for fluent API support)
     */
    public function setMedicines(Collection $medicines, ConnectionInterface $con = null)
    {
        /** @var ChildMedicine[] $medicinesToDelete */
        $medicinesToDelete = $this->getMedicines(new Criteria(), $con)->diff($medicines);


        $this->medicinesScheduledForDeletion = $medicinesToDelete;

        foreach ($medicinesToDelete as $medicineRemoved) {
            $medicineRemoved->setPrescription(null);
        }
    ...
    /**
     * Returns the number of related Medicine objects.
     *
     * @param      Criteria $criteria
     * @param      boolean $distinct
     * @param      ConnectionInterface $con
     * @return int             Count of related Medicine objects.
     * @throws PropelException
     */
    public function countMedicines(Criteria $criteria = null, $distinct = false, ConnectionInterface $con = null)
    {
        $partial = $this->collMedicinesPartial && !$this->isNew();
        if (null === $this->collMedicines || null !== $criteria || $partial) {
            if ($this->isNew() && null === $this->collMedicines) {
            ...
            if ($partial && !$criteria) {
                return count($this->getMedicines());
            }

            $query = ChildMedicineQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
        ...
        return count($this->collMedicines);
    }

    /**
     * Method called to associate a ChildMedicine object to this object
     * through the ChildMedicine foreign key attribute.
     *
     * @param  ChildMedicine $l ChildMedicine
     * @return $this|\Prescription The current object (for fluent API support)
     */
    public function addMedicine(ChildMedicine $l)
    {
        if ($this->collMedicines === null) {
            $this->initMedicines();
            $this->collMedicinesPartial = true;
        ...
        if (!$this->collMedicines->contains($l)) {
            $this->doAddMedicine($l);

            if ($this->medicinesScheduledForDeletion and $this->medicinesScheduledForDeletion->contains($l)) {
                $this->medicinesScheduledForDeletion->remove($this->medicinesScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param ChildMedicine $medicine The ChildMedicine object to add.
     */
    protected function doAddMedicine(ChildMedicine $medicine)
    {
        $this->collMedicines[]= $medicine;
        $medicine->setPrescription($this);
    }

    /**
     * @param  ChildMedicine $medicine The ChildMedicine object to remove.
     * @return $this|ChildPrescription The current object (for fluent API support)
     */
    public function removeMedicine(ChildMedicine $medicine)
    {
        if ($this->getMedicines()->contains($medicine)) {
            $pos = $this->collMedicines->search($medicine);
            $this->collMedicines->remove($pos);
            if (null === $this->medicinesScheduledForDeletion) {
                $this->medicinesScheduledForDeletion = clone $this->collMedicines;
                $this->medicinesScheduledForDeletion->clear();
            }
            $this->medicinesScheduledForDeletion[]= $medicine;
    ...
    public function clear()
    {
        ...
        if (null !== $this->aEmployee) {
            $this->aEmployee->removePrescription($this);
        }
        $this->id = null;
    ...
    {
        if ($deep) {
            if ($this->collMedicines) {
                foreach ($this->collMedicines as $o) {
