...
abstract class Payment implements ActiveRecordInterface
{
    ...
    public function setBillId($v)
    {
        ...
        if ($this->bill_id !== $v) {
            $this->bill_id = $v;
            $this->modifiedColumns[PaymentTableMap::COL_BILL_ID] = true;
        }

        if ($this->aBill !== null && $this->aBill->getId() !== $v) {
            $this->aBill = null;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : PaymentTableMap::translateFieldName('BillId', TableMap::TYPE_PHPNAME, $indexType)];
            $this->bill_id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : PaymentTableMap::translateFieldName('Amount', TableMap::TYPE_PHPNAME, $indexType)];
            $this->amount = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : PaymentTableMap::translateFieldName('Type', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    {
        if ($this->aBill !== null && $this->bill_id !== $this->aBill->getId()) {
            $this->aBill = null;
        }
    } // ensureConsistency
    ...
    protected function doSave(ConnectionInterface $con)
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->aBill !== null) {
                if ($this->aBill->isModified() || $this->aBill->isNew()) {
                    $affectedRows += $this->aBill->save($con);
                }
                $this->setBill($this->aBill);
            }
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(PaymentTableMap::COL_BILL_ID)) {
    ...
    public function toArray($keyType = TableMap::TYPE_PHPNAME, $includeLazyLoadColumns = true, $alreadyDumpedObjects = array(), $includeForeignObjects = false)
    {
        ...
        if ($includeForeignObjects) {
            if (null !== $this->aBill) {
                ...
                $result[$key] = $this->aBill->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
            }
        }
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(PaymentTableMap::COL_BILL_ID)) {
            $criteria->add(PaymentTableMap::COL_BILL_ID, $this->bill_id);
        }
        if ($this->isColumnModified(PaymentTableMap::COL_AMOUNT)) {
    ...
    /**
     * Declares an association between this object and a ChildBill object.
     *
     * @param  ChildBill $v
     * @return $this|\Payment The current object (for fluent API support)
     * @throws PropelException
     */
    public function setBill(ChildBill $v = null)
    {
        if ($v === null) {
            $this->setBillId(NULL);
        } else {
            $this->setBillId($v->getId());
        }

        $this->aBill = $v;

        // Add binding for other direction of this n:n relationship.
        // If this object has already been added to the ChildBill object, it will not be re-added.
        if ($v !== null) {
            $v->addPayment($this);
        }
    ...
    /**
     * Get the associated ChildBill object
     *
     * @param  ConnectionInterface $con Optional Connection object.
     * @return ChildBill The associated ChildBill object.
     * @throws PropelException
     */
    public function getBill(ConnectionInterface $con = null)
    {
        if ($this->aBill === null && ($this->bill_id != 0)) {
            $this->aBill = ChildBillQuery::create()->findPk($this->bill_id, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aBill->addPayments($this);
             */
        }
